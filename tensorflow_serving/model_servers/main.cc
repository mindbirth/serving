...
int main(int argc, char** argv) {
  ...
  std::vector<tensorflow::Flag> flag_list = {
      ...
      tensorflow::Flag("librarypath", &options.librarypath, "Load additional operators from given user_op library")
    };
  ...
  if (options.librarypath.size() > 0) {
    ...
    if (!TF_GetCode(status) == TF_OK) {
        ...
        std::cout << "Problem loading user_op library " <<  options.librarypath << ": " << TF_Message(status);
        return -1;
    }
